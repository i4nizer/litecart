<?php
  ...
  if (!empty($_POST['create_account'])) {
    ...
    try {
      ...
      # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
      # send custom email for email confirmation
      # because the lib seems to cache

      $token = bin2hex(random_bytes(16));
      $customerId = $customer->data['id'];
      $confirmation_link = document::ilink('confirm_email') . '?token=' . $token . '&id=' . $customer->data['id'];

      database::query("update " . DB_TABLE_PREFIX . "customers set email_confirm_token = '$token' where id = $customerId");
      ...
      $customMsg = "Kindly click the link to confirm your email and continue to homepage.\r\n\r\n$confirmation_link";
      mail($_POST['email'], $subject, $customMsg, $customHeaders);

      customer::reset();  # don't login yet
      ...
      header('Location: '. document::ilink('confirm_email'));
      exit;
